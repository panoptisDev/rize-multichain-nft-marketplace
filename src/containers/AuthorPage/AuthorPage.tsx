...
import default_banner from "../../images/profile/default_banner.png";
import twitterSvg from "../../images/profile/telegram.svg";
import dotsSvg from "../../images/profile/dots.svg";
import CustomViewIcon from "../../images/profile/custom_view_icon.jsx";
import ForSaleViewIcon from "../../images/profile/for_sale_view_icon.jsx";
import LikedViewIcon from "../../images/profile/liked_view_icon.jsx";
import FollowersIcon from "../../images/profile/followers_icon.jsx";
import FollowingsIcon from "../../images/profile/folllowings_icon";
...
const AuthorPage: FC<AuthorPageProps> = ({ className = "" }) => {
  ...
  const getIsExists = (user_id: string, target_id: string) => {
    ...
    axios
      .post(
        `${config.API_URL}api/follow/get_isExists`,
        { user_id, target_id },
        {
          headers: {
  ...
  const getDetailedUserInfo = async (userId: string, isForMine = true) => {
    ...
    await axios
      .post(
        `${config.API_URL}api/users/findOne`,
        { userId },
        {
          headers: {
            "x-access-token": localStorage.getItem("jwtToken"),
  ...
  const getItemsOfUserByConditions = (
    params: UserItemFetchingParams,
    userId: string
  ) => {
    if (isEmpty(userId)) return;
    axios
      .post(
        `${config.API_URL}api/item/get_items_of_user`,
      ...
      .then((result) => {
        ...
        if (activeIndex === PROFILE_TABS.COLLECTIBLES)
          setCollectedItems(result.data?.data ? result.data.data : []);
        if (activeIndex === PROFILE_TABS.CREATED)
          setCreatedItems(result.data?.data ? result.data.data : []);
        if (activeIndex === PROFILE_TABS.LIKED)
  ...
  const renderItem = (item: SocialType, index: number) => {
    ...
    return (
      <a
        href={item.href}
        className="flex items-center opacity-70 text-2xl text-[#33FF00] leading-none space-x-2 group w-[20px] h-[20px]"
        key={index}
      >
  ...
  return (
    <div className={`nc-AuthorPage mt-4  ${className}`} data-nc-id="AuthorPage">
      <Helmet>
