    ...
    >
      ...
      >
        {isVideo(nftItem?.logoURL) === false ? (
          ...
          <VideoForPreview
            ...
            nftId={nftItem?._id || DEMO_NFT_ID}
            className="flex aspect-w-12 aspect-h-10 w-full max-h-[250px] rounded-3xl overflow-hidden z-0 "
          />
      ...
      <div className="w-full transform -mt-[80px] relative z-10">
        ...
        <div className="block p-4 py-5 space-y-3 rounded-tl-none shadow-xl dark:transparent dark:shadow-2xl rounded-3xl">
          ...
          {!props?.isHome && (
            <div
              className="w-full flex justify-between items-end cursor-pointer"
              onClick={() => {
                (nftItem as any)?._id
                  ? navigate(`/nft-detail/${(nftItem as any)?._id}`)
                  : navigate("/nft-detail");
