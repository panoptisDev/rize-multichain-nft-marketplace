...
const SectionMainSlider = () => {
  const [rizeMemeberCollectionId, setRizeMemberCollectionId] = useState("6460aff6565e58a809f2f414")
  const navigate = useNavigate();
  const getRizeMemberCollectionId = () => {
    axios.post(
      `${config.API_URL}api/collection/getRizeMemberCollectionId`,
    ).then((response) => {
  ...
  return (
    <>
      <div className="relative flex flex-col items-center justify-center mt-1 mb-6">
        <p className="text-4xl font-semibold mb-4 text-neutral-900 dark:text-white">Rize Member NFTs</p>
        ...
        <div className="relative flex items-center justify-center gap-5">
          <ButtonPrimary onClick={() => navigate('collectionItems/' + rizeMemeberCollectionId)} className='w-[220px] rounded-3xl'>RIZE MEMBER NFTs</ButtonPrimary>
          <ButtonPrimary onClick={() => navigate('page-search')} className='w-[220px] rounded-3xl'>VIEW ALL NFTs</ButtonPrimary>
        </div>
